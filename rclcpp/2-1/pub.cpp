...
#include "std_msgs/msg/int32.hpp" // 발행할 메시지 타입
...
rclcpp::Publisher<std_msgs::msg::Int32>::SharedPtr mypub) // 100ms마다 호출되어 메시지를 발행하는 콜백 함수
{
    static auto message = std_msgs::msg::Int32(); // 발행할 Int32 메시지 객체 생성 및 초기화
    RCLCPP_INFO(node->get_logger(), "Publish: %d", message.data++); // ROS 2 로깅 시스템을 사용하여 현재 발행하는 데이터를 출력
...
int main(int argc, char* argv[])
{
    ...
    auto pub = node->create_publisher<std_msgs::msg::Int32>("mytopic", qos_profile); // topic설정
