...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<rclcpp::Node>("campub_7");
    auto qos_profile = rclcpp::QoS(rclcpp::KeepLast(10)); //TCP
    //auto qos_profile = rclcpp::QoS(rclcpp::KeepLast(10)).best_effort(); //UDP
    auto mypub = node->create_publisher<sensor_msgs::msg::CompressedImage>("image/compressed_7", qos_profile );
    ...
    while(rclcpp::ok())
    {
        ...
        msg = cv_bridge::CvImage(hdr, "bgr8", frame(cv::Rect(0, 270, 640, 90))).toCompressedImageMsg();
        mypub->publish(*msg);
        loop_rate.sleep();
    }
    rclcpp::shutdown();
